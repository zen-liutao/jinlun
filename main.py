...
var = tk.StringVar()
    ...
    # if file_name != '':
    #     label.config(text = '你选择的文件是： ' + file_name)
    # else:
    #     label.config(text ='您没有选择任何文件夹')
...
def main():
    ...
    if not validFilename(file_name):
        tkMessageBox.showwarning(title='警告',message='请选择正确的文件类型，后缀是.xlsx')
    else:
        try:
            ...
            wb.save(getDesktopPath() + '/' + "结果_"+file_name)
            tkMessageBox.showinfo(title='处理成功',message='任务处理成功')
        except Exception as e:           
            label.config(text=str(e))
            # print(e)
            tkMessageBox.showerror(title='失败',message='任务处理失败')


button = tk.Button(root,text="请选择文件", command=main)
button.pack()

info = tk.Label(root,justify='left', text='1.程序处理完成后，会保存在桌面上\n2.程序保存的文件名字叫结果_XXX\n3.文件只要有一个表格就行\n4.请使用.xlsx格式')
info.pack()

log = tk.Label(root,text='错误日志')
